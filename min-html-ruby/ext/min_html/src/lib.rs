...
use min_html::minify as min_html_native;
use min_html::Cfg as CfgNative;

fn min_html(source: String, cfg: RHash) -> String {
  #[rustfmt::skip]
  let out_code = min_html_native(source.as_bytes(), &CfgNative {
    ...
    preserve_self_closing_on_unknown_tags: cfg.aref(StaticSymbol::new("preserve_self_closing_on_unknown_tags")).unwrap_or_default(),
...
#[magnus::init]
fn init() {
  define_global_function("min_html", function!(min_html, 2))
}
