...
fn minify(mut cx: FunctionContext) -> JsResult<JsBuffer> {
  ...
  let cfg = min_html::Cfg {
    ...
    preserve_self_closing_on_unknown_tags: get_bool!(cx, opt, "preserve_self_closing_on_unknown_tags"),
  ...
  let out = min_html::minify(src.as_slice(&cx), &cfg);
